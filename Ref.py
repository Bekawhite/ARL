...
class SHAMember(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    sha_member_number = Column(String, unique=True)
    national_id = Column(String)
...
class Database:
    ...
    def get_all_bed_capacities(self):
        return self.session.query(BedCapacity).all()

    def update_bed_capacity(self, hospital_name, data, updated_by='system'):
        cap = self.session.query(BedCapacity).filter(BedCapacity.hospital_name == hospital_name).first()
        if cap:
    ...
    def get_capacity_status(self, hospital_name):
        ...
        if not cap or cap.total_beds == 0:
            return 'unknown', 0
        occupancy_pct = (cap.occupied_beds / cap.total_beds) * 100
        if occupancy_pct >= 90:
            return 'red', occupancy_pct
    ...
    def create_sha_claim(self, patient_id, ambulance_id, distance_km):
        if distance_km <= Config.SHA_BASE_DISTANCE_KM:
            ...
            base_charge = Config.SHA_BASE_CHARGE_KES
            extra_km = distance_km - Config.SHA_BASE_DISTANCE_KM
            additional_charge = extra_km * Config.SHA_PER_KM_CHARGE_KES
        ...
        if patient:
            patient.sha_claim_id = claim_id
            patient.sha_billing_amount_kes = round(total_amount, 2)
            patient.sha_distance_km = round(distance_km, 2)
            patient.sha_claim_status = 'Submitted'
        self.session.commit()
        return claim
    ...
    def get_pending_offline_actions(self):
        return self.session.query(OfflineQueue).filter(OfflineQueue.synced == False).all()

    def mark_offline_synced(self, item_id, error=None):
        item = self.session.query(OfflineQueue).filter(OfflineQueue.id == item_id).first()
        if item:
            item.synced = True if not error else False
...
class BedCapacityService:
    ...
    def get_capacity_badge(self, hospital_name):
        ...
        if status == 'red':
            ...
            return f"🟡 {pct:.0f}% full", "amber"
        elif status == 'green':
            return f"🟢 {pct:.0f}% full", "green"
...
class FHIRService:
    ...
    @staticmethod
    def export_bundle(patient, ambulance=None):
        bundle = {
            "resourceType": "Bundle",
            "type": "transaction",
    ...
    @staticmethod
    def render_fhir_panel(patient, ambulance=None):
        ...
        with col1:
            if st.button("📄 Export FHIR Bundle", key=f"fhir_bundle_{patient.patient_id}", use_container_width=True):
                ...
                st.download_button(
                    ...
                    key=f"fhir_dl_{patient.patient_id}"
                )
...
class MOHReferralLetterService:
    ...
    def generate_moh_referral_letter(self, patient, referring_hospital_data, receiving_hospital_data, ambulance=None):
        ...
        if ambulance:
            fac_data.append([
                Paragraph("<b>Ambulance ID:</b>", label_style),
                Paragraph(ambulance.ambulance_id, value_style),
...
class TriageService:
    ...
    @staticmethod
    def score_to_triage(mews_score):
        if mews_score >= 5:
            return 'Red'
        elif mews_score >= 3:
            return 'Orange'
        ...
        if recent_patients:
            ...
            for patient in recent_patients:
                ...
                data.append({
                    'Patient ID': patient.patient_id,
                    'Name': patient.name,
                    'Triage': f"{triage_icon} {patient.triage_level}",
                    'Condition': patient.condition,
        ...
        if active_patients:
            ...
            for patient in active_patients:
                ...
                data.append({
                    ...
                    'MEWS': patient.mews_score or 0,
                    'Condition': patient.condition,
                    'From': patient.referring_hospital,
                    'To': patient.receiving_hospital,
                    'Status': patient.status,
                    'SHA': '✅ Verified' if patient.sha_verified else '❌ Unverified',
            ...
            for patient in active_patients:
                triage_icon = {'Red': '🔴', 'Orange': '🟠', 'Green': '🟢'}.get(patient.triage_level, '⚪')
                with st.expander(f"{triage_icon} Actions for {patient.name} ({patient.patient_id})"):
        ...
        if filtered_patients:
            ...
            for patient in filtered_patients:
                ...
                data.append({
                    ...
                    'MOH Ref': patient.moh_referral_number or 'N/A',
                    'Referral Time': patient.referral_time.strftime('%Y-%m-%d %H:%M'),
...
class SHABillingUI:
    ...
    def display(self):
        ...
        with tab1:
            self.display_claims()
        with tab2:
            self.display_tariff_calculator()
        with tab3:
            self.display_sha_members()
